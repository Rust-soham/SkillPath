...
export default function SkillpathCourses({ heading, accent, style }: Props) {
    ...
    return <section className="sp" style={{ ...style, "--accent": accent } as CSSProperties}>
        ...
        {courses && courses.length > 0 && <><div className="sp-tools"><input className="sp-field" type="search" value={search} onChange={event => setSearch(event.currentTarget.value)} placeholder="Search skills, courses, or categories" aria-label="Search courses"/><select className="sp-field" value={sort} onChange={event => setSort(event.currentTarget.value)} disabled={!country}><option value="featured">Featured order</option><option value="low">Price: low to high</option><option value="high">Price: high to low</option></select></div>{countryError && <p className="sp-note">Regional pricing is unavailable, so both currencies are shown. <button onClick={load}>Retry</button></p>}</>}
