...
.sp-field,.sp-button,.sp-note button{font-family:inherit}.sp-control{display:flex;min-width:0;flex-direction:column;gap:8px}.sp-control-label{color:var(--muted);font:700 10px/1 monospace;letter-spacing:.08em;text-transform:uppercase}.sp-field{font-size:14px;font-weight:500}.sp-note button{margin-left:6px;padding:3px 9px;border:1px solid var(--accent);border-radius:3px;color:var(--ink);background:#fff;font-size:13px;font-weight:600;line-height:1.1;cursor:pointer}@container(max-width:599px){.sp-tools{gap:16px;margin-bottom:24px}}
`
...
export default function SkillpathCourses({ heading, accent, style }: Props) {
    ...
    return <section className="sp" style={{ ...style, "--accent": accent } as CSSProperties}>
        ...
        {courses && courses.length > 0 && <><div className="sp-tools"><label className="sp-control"><span className="sp-control-label">Search courses</span><input className="sp-field" type="search" value={search} onChange={event => setSearch(event.currentTarget.value)} placeholder="Skills, courses, or categories"/></label><label className="sp-control"><span className="sp-control-label">Sort courses</span><select className="sp-field" value={sort} onChange={event => setSort(event.currentTarget.value)} disabled={!country}><option value="featured">Featured</option><option value="low">Price: low to high</option><option value="high">Price: high to low</option></select></label></div>{countryError && <p className="sp-note">Regional pricing is unavailable, so both currencies are shown. <button onClick={load}>Retry</button></p>}</>}
